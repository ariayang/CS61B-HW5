 ...
 **/
public class Set extends SList {
  ...
  public Set() { 
    // Your solution here.
      alist = new SList();  //Why can't i use 'this' for the code below? Compiling erros.
  }
  ...
  public void union(Set s) {
    // Your solution here.
      ...
      if (!s.isEmpty()){
	  try {
	      ListNode scursor = s.front(); //cursor in Set s
	      ListNode acursor = scursor; // cursor in alist, to be updated once s insert items
	      while (acursor != a.back()) {
		  if (scursor.item().compareTo(acursor.item()) < 0) {
		      alist.insertBefore(scursor.item());
